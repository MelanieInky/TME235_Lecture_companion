...
.viz-poisson,
[data-bs-theme="light"] .viz-poisson,
body.quarto-light .viz-poisson {
...
[data-bs-theme="dark"] .viz-poisson,
body.quarto-dark .viz-poisson {
  --fg: #f0f0f0; --fg-muted: #a0a0a3;
  --panel-bg: #262628; --border: #3a3a3c; --grid: #38383a;
  --slider: #9a9a9d;
}
...
<div class="subtitle">Pull a bar along its length — ν sets how much it shrinks (or grows) sideways</div>
  ...
  <div class="controls">
    <div class="slider-group">
      <label for="po-eps">Applied axial strain ε <span class="val" id="po-vEps">0.20</span></label>
      <input type="range" id="po-eps" min="0" max="0.4" step="0.01" value="0.20">
    </div>
    <div class="slider-group">
      <!-- step must be fine enough to hit the ν = 0.499 preset; at step="0.01"
           the browser snapped it to 0.50 and duplicated the incompressible button. -->
      <label for="po-nu">Poisson's ratio ν <span class="val" id="po-vNu">0.300</span></label>
      <input type="range" id="po-nu" min="-0.9" max="0.5" step="0.001" value="0.300">
    </div>
  </div>

  <div class="presets">
    <button data-nu="0.300">Typical metal (ν≈0.30)</button>
    <button data-nu="0.000">Cork (ν≈0.00)</button>
    <button data-nu="0.499">Rubber (ν≈0.499)</button>
    <button data-nu="0.500">Perfectly incompressible (ν=0.5)</button>
    <button data-nu="-0.500">Auxetic foam (ν≈−0.5)</button>
  </div>

  <div class="stage">
    <svg id="po-svg-bar" viewBox="0 50 480 160"></svg>
    <div class="readouts">
      <div class="item"><div class="label">Axial strain ε</div><div class="value" id="po-r-ax"></div></div>
      <div class="item"><div class="label">Lateral strain ε_lat = −νε</div><div class="value" id="po-r-lat"></div></div>
      <div class="item"><div class="label">Width W/W₀</div><div class="value" id="po-r-wid"></div></div>
      <div class="item"><div class="label">Volume change ΔV/V₀ = (1−2ν)ε</div><div class="value" id="po-r-vol"></div></div>
    </div>
  </div>
  ...
  <div class="note" id="po-explainer">
    ...
    The volume readout is the linearised ΔV/V₀ = tr(ε) = (1−2ν)ε, so ν=0.5 gives exactly zero; the
    drawing stretches the bar by the finite factors (1+ε) and (1+ε_lat), which differ from the
    linearised picture by second-order terms at the large strains these sliders allow.
  </div>
...
function update() {
  ...
  // Linearised volume change: tr(eps) = eps + 2*epsLat = (1 - 2nu)*eps.
  // The finite product (1+eps)(1+epsLat)^2 - 1 was reporting ~-2.8% at nu = 0.5,
  // contradicting the note directly below the figure.
  const dV = (1 - 2 * nu) * eps;

  __q('po-r-ax').textContent  = `${(eps*100).toFixed(1)}%`;
  __q('po-r-lat').textContent = `${(epsLat*100).toFixed(1)}%`;
  __q('po-r-wid').textContent = (1 + epsLat).toFixed(3);
  const volEl = __q('po-r-vol');
  volEl.textContent = `${(dV*100).toFixed(1)}%`;
  volEl.style.color = Math.abs(dV) < 5e-4 ? 'var(--fg)' : (dV > 0 ? 'var(--grow)' : 'var(--shrink)');
