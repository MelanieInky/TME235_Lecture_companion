...
.viz-strain,
[data-bs-theme="light"] .viz-strain,
body.quarto-light .viz-strain {
  --fg: #1a1a1a; --fg-muted: #6b6b6b;
  --panel-bg: #f7f7f8; --border: #e2e2e4; --grid: #e8e8ea;
  --slider: #555;
  --accent-sph: #2E86AB; --accent-dev: #C1440E; --accent-tot: #3E8914;
}
@media (prefers-color-scheme: dark) {
  .viz-strain {
    --fg: #f0f0f0; --fg-muted: #a0a0a3;
    --panel-bg: #262628; --border: #3a3a3c; --grid: #333335;
    --slider: #9a9a9d;
  }
}
[data-bs-theme="dark"] .viz-strain,
body.quarto-dark .viz-strain {
  --fg: #f0f0f0; --fg-muted: #a0a0a3;
  --panel-bg: #262628; --border: #3a3a3c; --grid: #333335;
  --slider: #9a9a9d;
}
...
.viz-strain .subtitle {
  text-align: center; color: var(--fg-muted); font-size: 0.9rem; margin-bottom: 24px;
}
.viz-strain .controls {
...
.viz-strain .slider-group {
  ...
  min-width: 210px; flex: 1 1 210px; max-width: 300px;
}
...
.viz-strain .panel {
  background: var(--panel-bg); border: 1px solid var(--border); border-radius: 12px;
  padding: 12px 12px 16px; text-align: center; overflow-x: auto;
}
.viz-strain .panel h2 {
  font-size: 0.92rem; font-weight: 600; margin: 4px 0 2px;
}
.viz-strain .panel .desc {
  font-size: 0.78rem; color: var(--fg-muted); margin-bottom: 8px;
}
.viz-strain .panel .area {
  font-size: 0.78rem; color: var(--fg-muted); margin-top: 6px;
}
.viz-strain svg {
...
.viz-strain .matrix-row {
  ...
  margin-top: 22px; font-size: 0.85rem;
}
...
.viz-strain .note {
  max-width: 640px; margin: 26px auto 0; font-size: 0.8rem; color: var(--fg-muted);
...
<div class="subtitle">ε = ε<sub>sph</sub> + ε<sub>dev</sub> — drag the sliders to set ε₁₁, ε₂₂, ε₁₂</div>
  ...
  <div class="controls">
    <div class="slider-group">
      <label for="sd-e11">ε₁₁ <span class="val" id="sd-v11">0.18</span></label>
      <input type="range" id="sd-e11" min="-0.4" max="0.4" step="0.01" value="0.18">
    </div>
    <div class="slider-group">
      <label for="sd-e22">ε₂₂ <span class="val" id="sd-v22">-0.06</span></label>
      <input type="range" id="sd-e22" min="-0.4" max="0.4" step="0.01" value="-0.06">
    </div>
    <div class="slider-group">
      <label for="sd-e12">ε₁₂ = ε₂₁ <span class="val" id="sd-v12">0.10</span></label>
  ...
  <div class="presets">
    ...
    <button data-e="0.2,-0.2,0">Pure deviatoric (equal and opposite)</button>
  ...
  <div class="matrix-row">
    ...
    <div class="matrix-block">
      <div class="label">ε<sub>sph</sub> = ε<sub>v</sub>I, ε<sub>v</sub> = ½tr(ε) <b>in 2D</b></div>
  ...
  <div class="note">
    This is the <b>two-dimensional</b> split, so the spherical part carries ε<sub>v</sub> = ½tr(ε);
    in 3D the same construction uses ⅓tr(ε), and it's a common slip to carry the ½ across.
    Axes are drawn maths-convention (x₁ right, x₂ <i>up</i>), so a positive ε₁₂ tilts the square
    the way it does in a textbook figure. Squares are drawn with the finite map x′ = (I + ε)x for
    visual clarity, not the strict infinitesimal linearization — so at large strain the
    "area-preserving" property of the deviatoric part is only approximate (visible as small drift
...
function drawPanel(svgId, e, color) {
  ...
  // The polygons are computed in maths coordinates (x2 up) and flipped once here,
  // because SVG's y axis points down — without this a positive eps12 renders as the
  // mirror image of the textbook picture, which the drawn axes invite you to read off.
  svg.innerHTML = `
...
function fmt(x) {
  // Snap near-zero to zero so the table never shows "-0.000", and pad positives with a
  // non-breaking space (a plain space collapses in HTML and the sign column drifts).
...
function update() {
  ...
  const ev = 0.5 * (e11 + e22);   // 2D: half the trace (3D would be a third)
  const sph = [[ev, 0], [0, ev]];
